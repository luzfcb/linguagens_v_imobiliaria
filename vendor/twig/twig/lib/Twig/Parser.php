    ...
    protected function filterBodyNodes(Twig_NodeInterface $node)
    {
        ...
        if (
            ...
            if (false !== strpos((string) $node, chr(0xEF).chr(0xBB).chr(0xBF))) {
                throw new Twig_Error_Syntax('A templates that extends another one cannot have a body but a byte order mark (BOM) has been detected; it must be removed.', $node->getLine(), $this->stream->getFilename());
            } else {
                throw new Twig_Error_Syntax('A templates that extends another one cannot have a body.', $node->getLine(), $this->stream->getFilename());
            }
        }
